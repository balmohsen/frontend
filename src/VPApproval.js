...
const VPApproval = ({ vpId }) => {
    ...
    useEffect(() => {
        const fetchCertificates = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/certificates/vp/${vpId}`, {
                    headers: {
    ...
    const handleAction = async (id, action) => {
        ...
        try {
            const response = await axios.patch(`${process.env.REACT_APP_API_URL}/approve/vp/${id}`, { action }, {
                headers: {
                    Authorization: `Bearer ${auth.token}`,
                },
            });
    ...
    if (loading) {
        return <p>Loading VP Dashboard...</p>;
    }
