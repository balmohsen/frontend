// src/ManagerApproval.js

import React, { useEffect, useState, useContext } from 'react';
import axios from 'axios';
import { AuthContext } from './context/AuthContext';

const ManagerApproval = ({ managerId }) => {
    ...
    useEffect(() => {
        const fetchCertificates = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/certificates/manager/${managerId}`, {
                    headers: {
    ...
    const handleAction = async (id, action) => {
        ...
        try {
            const response = await axios.patch(`${process.env.REACT_APP_API_URL}/approve/manager/${id}`, { action }, {
                headers: {
                    Authorization: `Bearer ${auth.token}`,
                },
            });
